...
class pose_model(nn.Module):
    ...
    def forward(self, x):
        ...
        # out2_1 = self.model2_1(out2)
        # out2_2 = self.model2_2(out2)
        # out3 = torch.cat([out1_1, out2_2, out1], 1)

        # out3_1 = self.model3_1(out3)
        # out3_2 = self.model3_2(out3)
        # out4 = torch.cat([out1_1, out3_2, out1], 1)

        # out4_1 = self.model4_1(out4)
        # out4_2 = self.model4_2(out4)
        # out5 = torch.cat([out1_1, out4_2, out1], 1)

        # out5_1 = self.model5_1(out5)
        # out5_2 = self.model5_2(out5)
        # out6 = torch.cat([out1_1, out5_2, out1], 1)

        # out6_1 = self.model6_1(out6)
        # out6_2 = self.model6_2(out6)

        return out1_1, out1_2
        # return out1_1, out6_2
...
toc = time.time()
print 'reading image time is %.5f' % (toc - tic)
tic = time.time()
...
toc = time.time()
print 'Model running time is %.5f' % (toc - tic)
tic = time.time()
...
toc = time.time()
print 'heatmap avg time is %.5f' % (toc - tic)
tic = time.time()
...
toc = time.time()
print '\"Everything else\" time taken was %.5f' % (toc - tic)
fp = 'result_opt.png'
print 'writing file to {}'.format(fp)
